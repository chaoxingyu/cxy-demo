...
public class SftpUtil {
     ...
     * @param port       端口
     */
    public SftpUtil(String userName, String passWord, String privateKey, String host, int port) throws Exception {
    ...
    private ChannelSftp connect() throws Exception {
        ...
        if (null != privateKey) {
            ...
            LOGGER.info("SFTP connect by privateKey, path of private key file：{}", privateKey);
        }
        LOGGER.info("SFTP connect by host:{} username:{}", host, userName);
        session = jsch.getSession(userName, host, port);
        LOGGER.info("SFTP Session is build");
        if (null != passWord) {
        ...
        LOGGER.info("SFTP server host:{} port:{} is connect success ", host, port);
    ...
    public void disConnect() {
        ...
        LOGGER.info("sftp is already closed");
    }
    ...
    private boolean dirIsExist(String dir) {
        try {
            ...
            if (attrs.isDir()) {
                ...
                LOGGER.info("远程[{}] 不是目录！", dir);
            }
        } catch (Exception e) {
            LOGGER.error("远程目录[{}]不存在！", dir);
            LOGGER.error("判断远程目录是否存在, 异常：", e);
        }
        return false;
    }
    ...
    private boolean fileIsExist(String remoteFile) {
        try {
            ...
            LOGGER.info("远程[{}] 不是文件！", remoteFile);
        } catch (Exception e) {
            LOGGER.error("远程文件[{}] 不存在！", remoteFile);
            LOGGER.error("判断远程文件是否存在 异常：", e);
        }
        return false;
    }
    ...
    public boolean uploadFile(String localPath, String localFileName, String remotePath, String remoteFileName) throws Exception {
        LOGGER.info("Sftp文件上传--->本地文件[{},{}]，远程目录[{},{}]--->开始！", localPath, localFileName, remotePath, remoteFileName);
        // 判断本地文件是否存在
        String filePath = FileUtil.getAbsolutePath(localPath) + localFileName;
        ...
        LOGGER.info("Sftp文件上传--->本地文件[{},{}]，远程目录[{},{}]--->成功！", localPath, localFileName, remotePath, remoteFileName);
        return true;
    }
     ...
     * @throws Exception
     */
    public void uploadFile(InputStream input, String remoteFileName, String remotePath) throws Exception {
        LOGGER.info("Sftp文件上传---文件流-->本地文件[{}], 远程目录[{}]--->开始！", remoteFileName, remotePath);
        try {
            createRemoteDir(remotePath);
            this.sftp.put(input, remoteFileName);
            LOGGER.info("Sftp文件上传---文件流-->本地文件[{}], 远程目录[{}]--->成功！", remoteFileName, remotePath);
        } finally {
            if (null != input) {
                try {
                    input.close();
                } catch (Exception e) {
                    LOGGER.info("Sftp文件上传关闭文件流时异常，", e);
                }
            }
        }
    }
     ...
     * @throws Exception
     */
    public boolean uploadBatchFiles(String remotePath, String localPath, boolean isDel) throws Exception {
        LOGGER.info("Sftp批量文件上传--->本地目录[{}], 远程目录[{}]--->开始处理！", localPath, remotePath);
        File dirLocal = new File(localPath);
        File[] files = dirLocal.listFiles();
        if (null == files || 0 == files.length) {
            LOGGER.info("Sftp批量文件上传--->本地目录[{}] 无 文件！", localPath);
            return false;
        }
        for (File file : files) {
            if (file.isFile()) {
        ...
        LOGGER.info("Sftp批量文件上传--->本地目录[{}], 文件数量[{}], 远程目录[{}]--->上传成功！", localPath, files.length, remotePath);
        return true;
    ...
    private boolean deleteFile(String filePath) {
        ...
        LOGGER.info("delete file {} from local.", rs);
        return rs;
    }
    ...
    public boolean downloadFile(String remotePath, String remoteFileName, String localPath, String localFileName) throws Exception {
        LOGGER.info("Sftp文件下载--->远程目录[{}], 文件名称[{}], 本地目录[{},{}]--->开始处理！", remotePath, remoteFileName, localPath, localFileName);
        String fileRemotePath = FileUtil.getAbsolutePath(remotePath) + remoteFileName;
        String fileLocalPath = FileUtil.getAbsolutePath(localPath) + localFileName;
        this.sftp.get(fileRemotePath, fileLocalPath);
        LOGGER.info("Sftp文件下载--->远程目录[{}], 文件名称[{}], 本地目录[{},{}]--->处理成功！", remotePath, remoteFileName, localPath, localFileName);
        return true;
    }
    ...
    public InputStream downFile(String remotePath, String fileName) throws Exception {
        LOGGER.info("Sftp文件下载到本地流--->远程目录[{}], 文件名称[{}]--->开始处理！", remotePath, fileName);
        ...
        LOGGER.info("Sftp文件下载到本地流--->远程目录[{}], 文件名称[{}]--->处理成功！", remotePath, fileName);
        return ins;
    }
     ...
     * @throws Exception
     */
    public List<String> downBatchFiles(String remotePath, String localPath, String fileFormat, String fileEndFormat, boolean isDel) throws Exception {
        LOGGER.info("SFTP批量下载文件 远程下载目录[{}], 本地保存目录[{}], 下载文件格式：{}, 下载文件格式：{}, 下载后是否删除sftp文件：{}  下载开始！", remotePath, localPath, fileFormat, fileEndFormat, isDel);
        ...
        try {
            ...
            LOGGER.info("SFTP 批量下载文件个数:{}", v.size());
            if (v.isEmpty()) {
                return fileNameList;
            }
            ...
            while (it.hasNext()) {
                ...
                if (!attrs.isDir()) {
                    ...
                    if (flag) {
                        LOGGER.info("SFTP 批量下载文件 下载单个文件[{}] 成功！", localFileName);
                    ...
                    LOGGER.info("SFTP 批量下载文件 远程下载目录：{} 下存在子目录：{} ", remotePath, fileName);
                }
            }
            LOGGER.info("SFTP 批量下载文件 远程下载目录：{}, 本地保存目录：{}, 下载文件格式：{}, 下载文件格式：{}, 下载后是否删除sftp文件：{}  下载成功！", remotePath, localPath, fileFormat, fileEndFormat, isDel);
        } catch (SftpException e) {
            LOGGER.error("SFTP 批量下载文件 异常: ", e);
        }
        return fileNameList;
    }
    ...
    public void downBatchFiles(String remotePath, String localPath, boolean isMonitor) throws Exception {
        LOGGER.info("Sftp批量下载文件--->远程目录[{}], 本地目录[{}]--->开始处理！", remotePath, localPath);
        if (!dirIsExist(remotePath)) {
            throw new Exception("Sftp批量下载文件 异常：远程目录[" + remotePath + "] 不存在！");
        }
        ...
        LOGGER.info("Sftp批量下载文件--->远程目录[{}], 本地目录[{}], 文件数量[{}]--->处理成功！", remotePath, localPath, list.size());
    }
    ...
    public boolean deleteSFTPFile(String dir, String fileName) throws SftpException {
        ...
        LOGGER.info("delete file[{}] success from sftp.", dir + fileName);
        return true;
    }
    ...
    private void mkLocalDirs(String path) {
        ...
        if (!pathDir.exists() && !pathDir.isDirectory()) {
            ...
            LOGGER.info("mkLocalDirs {} is {}", path, flag);
        }
    }
